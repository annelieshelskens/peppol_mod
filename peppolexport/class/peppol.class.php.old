...
/**
 * \file        class/peppol.class.php
 * \ingroup     peppolexport
 * \brief       This file is a CRUD class file for Peppol (Create/Read/Update/Delete)
 */
 ...
 * Class for Peppol
 */
class Peppol extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'peppolexport';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'peppol';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'peppolexport_peppol';

	/**
	 * @var string 	If permission must be checkec with hasRight('peppolexport', 'read') and not hasright('mymodyle', 'peppol', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'peppolexport';

	/**
	 * @var string 	String with name of icon for peppol. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'peppol@peppolexport' if picto is file 'img/object_peppol.png'.
	 */
	...
	public $fields = array(
		...
		"status" => array("type" => "integer", "label" => "Status", "enabled" => "1", 'position' => 2000, 'notnull' => 1, "visible" => "1", "index" => "1", "arrayofkeyval" => array("0" => "Concept", "1" => "Gevalideerd", "9" => "Geannuleerd"), "validate" => "1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'peppolexport_peppolline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_peppol';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Peppolline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Peppol', 'fk_element'=>'fk_peppol'));
	...
	//  */
	// protected $childtablesoncascade = array('peppolexport_peppoldet');

	// /**
	//  * @var PeppolLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('peppolexport', 'peppol', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('peppolexport', 'peppol', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('peppolexport', 'peppol_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'peppol/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'peppol/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'peppol/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'peppol/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->peppolexport->dir_output.'/peppol/'.$oldref;
				$dirdest = $conf->peppolexport->dir_output.'/peppol/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->peppolexport->dir_output.'/peppol/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('peppolexport','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('peppolexport','peppolexport_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'PEPPOLEXPORT_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'PEPPOLEXPORT_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'PEPPOLEXPORT_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPeppol")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Peppol").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/peppolexport/peppol_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new PeppolLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_peppol:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('PEPPOLEXPORT_MYOBJECT_ADDON')) {
			$conf->global->PEPPOLEXPORT_MYOBJECT_ADDON = 'mod_peppol_standard';
		}

		if (getDolGlobalString('PEPPOLEXPORT_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('PEPPOLEXPORT_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('PEPPOLEXPORT_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/peppolexport/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefPeppol $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/peppolexport/doc/";
...
/**
 * Class PeppolLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PeppolLine extends CommonObjectLine
{
	// To complete with content of an object PeppolLine
	// We should have a field rowid, fk_peppol and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'peppol'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_peppol'
