...
/**
 * \file    peppolexport/admin/setup.php
 * \ingroup peppolexport
 * \brief   PEPPOLexport setup page.
 */
...
require_once '../lib/peppolexport.lib.php';
...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('peppolexportsetup', 'globalsetup'));
...
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM1');
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('PEPPOLEXPORT_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('PEPPOLEXPORT_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('PEPPOLEXPORT_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('PEPPOLEXPORT_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('PEPPOLEXPORT_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('PEPPOLEXPORT_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('PEPPOLEXPORT_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf PEPPOLEXPORT_MYPARAM10
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('PEPPOLEXPORT_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/peppolexport/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=peppolexport-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'PEPPOLEXPORT_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'PEPPOLEXPORT_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "PEPPOLexportSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-peppolexport page-admin');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "peppolexport@peppolexport");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("PEPPOLexportSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'PEPPOLEXPORT_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'PEPPOLEXPORT_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
